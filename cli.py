...
def show(limit: int = 10):
    ...
    with Session(engine) as session:
        averages = session.exec(select(PollAverage).order_by(PollAverage.id.desc())).all()
        if averages:
            table = Table(title="Polling Averages")
            table.add_column("Type", style="cyan")
            table.add_column("Positive (%)", style="green")
            table.add_column("Negative (%)", style="red")
            table.add_column("Net", style="bold yellow")
            
            seen_types = set()
            for avg in averages:
                if avg.poll_type not in seen_types:
                    net_str = f"{'+' if avg.net_avg > 0 else ''}{avg.net_avg}"
                    table.add_row(avg.poll_type, str(avg.positive_avg), str(avg.negative_avg), net_str)
                    seen_types.add(avg.poll_type)
            console.print(table)

        polls = session.exec(select(Poll).order_by(Poll.id.desc()).limit(limit)).all()
        if polls:
